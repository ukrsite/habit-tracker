...
const NotificationToast = ({ notification, onDismiss }: { notification: Notification; onDismiss: () => void }) => {
  ...
  return (
    <div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-500 rounded-lg shadow-xl p-4 pointer-events-auto animate-slide-in">
    ...
    <div className="fixed top-4 right-4 z-50 flex flex-col gap-3 max-w-sm pointer-events-none">
      {notifications.map((notification) => (
        <NotificationToast
          ...
          onDismiss={() => onDismiss(notification.habitId, notification.milestoneDays)}
        />
      ))}
    </div>
  );
